...
void Solve_Buff::run_SolvePnp(Mat & srcImg, RotatedRect & rect, float buff_angle, int pre_angle){
    ...
    get_Angle(ptz, buff_angle, pre_angle);
}
...
void Solve_Buff::get_Angle(const Mat & pos_in_ptz, float buff_angle, int pre_angle){
    //计算子弹下坠补偿
...
//        float buff_robot_z = 7200;
        ...
        float dist = sqrt(pow(target_h, 2) + pow(BUFF_ROBOT_Z, 2));
        z = dist;
    }
    z = _xyz[2];
