...
bool BuffDetector::ImageProcess(Mat & img){
    ...
    double th = threshold(gaussImg, binImg, 15, 255,  0|8);
    if(th-10 > 0)
        threshold(gaussImg, binImg, th-10, 255,  0);
...
bool BuffDetector::FindCenterR(Mat &bin_img, Mat &img){
    ...
    for(int j = 0; j < (int)contours.size(); ++j){
        ...
        if(circle_area > 300 || circle_area < 50)
            continue;
        ...
        //cout << rect_ratio << endl;

        if(rect_ratio > 0.7f && rect_ratio < 1.3f){
...
int BuffDetector::BuffDetectTask(Mat &img){
    ...
    if(is_target){//可找到未激活目标
        RotatedRect roi_R(roi_center, Size2f(100,100), 0);
        ...
        roi_R.points(ver);
        for(int k=0;k<4;k++){
            line(img, ver[k],ver[(k+1)%4], Scalar(0, 0, 255), 3);
        }
